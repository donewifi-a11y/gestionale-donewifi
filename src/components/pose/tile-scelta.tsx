      ...
      {opzioni.map((o) => {
        ...
        return (
          <button
            ...
            type="button"
            onClick={() => onChange(o)}
            className={`flex h-16 items-center justify-between rounded-2xl border-2 px-5 text-left text-lg font-bold transition active:scale-[0.98] ${
              attivo ? "border-primary bg-primary/8 text-primary" : "border-border bg-background text-foreground"
            }`}
          >
            {o}
            <span
              className={`flex h-7 w-7 shrink-0 items-center justify-center rounded-full border-2 ${
                attivo ? "border-primary bg-primary text-primary-foreground" : "border-border"
              }`}
            >
...
export function CampoGrande(props: React.InputHTMLAttributes<HTMLInputElement>) {
  return (
    <input
      {...props}
      className={`h-16 w-full rounded-2xl border-2 border-border bg-background px-5 text-lg font-bold text-foreground placeholder:font-normal placeholder:text-muted-foreground focus:border-primary focus:outline-none ${props.className ?? ""}`}
    />
  );
}
...
export function AreaGrande(props: React.TextareaHTMLAttributes<HTMLTextAreaElement>) {
  return (
    <textarea
      {...props}
      className={`min-h-32 w-full rounded-2xl border-2 border-border bg-background px-5 py-4 text-base font-medium text-foreground placeholder:font-normal placeholder:text-muted-foreground focus:border-primary focus:outline-none ${props.className ?? ""}`}
    />
  );
}
