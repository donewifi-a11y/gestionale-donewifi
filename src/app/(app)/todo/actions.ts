...
// ★ FIX (2026-09-25, audit modulo Todo) — queste tre funzioni non
// controllavano affatto `getPersonaCorrenteId()`, né filtravano per
// `persona_id`: contavano solo sulla RLS ("aggiorna/elimina propri
// to-do", migrazione 0043). Un update/delete filtrato dalla RLS a 0 righe
// non torna come errore da PostgREST — stesso comportamento silenzioso
// già scoperto per Ticket e Lavorazioni Interne: un id non proprio (bug
// altrove, o un id fabbricato ad arte) avrebbe dato "fatto" senza che
// nulla fosse davvero cambiato. Aggiunto il controllo persona + il filtro
// esplicito su `persona_id`, e un controllo che la riga sia stata
// davvero trovata/modificata prima di dichiarare successo.
export async function completaTodoPersonale(id: string, fatto: boolean): Promise<{ errore: string | null }> {
  ...
  if (!data || data.length === 0) return { errore: "To-do non trovato." };
  return { errore: null };
}
...
export async function eliminaTodoPersonale(id: string): Promise<{ errore: string | null }> {
  ...
  const { data, error } = await supabase.from("todo_personali").delete().eq("id", id).eq("persona_id", personaId).select("id");
  if (error) return { errore: error.message };
  if (!data || data.length === 0) return { errore: "To-do non trovato." };
  return { errore: null };
}
...
export async function modificaTodoPersonale(id: string, testo: string): Promise<{ errore: string | null }> {
  ...
  const { data, error } = await supabase.from("todo_personali").update({ testo: testoPulito }).eq("id", id).eq("persona_id", personaId).select("id");
  if (error) return { errore: error.message };
  if (!data || data.length === 0) return { errore: "To-do non trovato." };
  return { errore: null };
}
